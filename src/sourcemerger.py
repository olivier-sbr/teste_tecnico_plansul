...
class SourceMerger:
    def __init__(self, df_excel: pd.DataFrame, df_csv: pd.DataFrame):
        ...
        self.df = df.drop(columns="_merge")

    def log_divergences(self) -> None:
        self._log_single_source()
        self._log_value_divergences()
        self._log_name_divergences()
        self._log_ans_divergences()
    ...
    def _log_single_source(self) -> None:
        for _, row in self.df[self.df["fonte"] != "BOTH"].iterrows():
            logger.warning(f"cobranca em fonte unica: {row['id_cobranca']} | fonte={row['fonte']}")

    def _log_value_divergences(self) -> None:
        both = self.df[self.df["fonte"] == "BOTH"].copy()
        both["delta"] = abs(both["valor"] - both["vl_liquido"] - both["vl_glosa"])
        for _, row in both[both["delta"] > 0.25].iterrows():
            logger.warning(
    ...
    def _log_name_divergences(self) -> None:
        both = self.df[self.df["fonte"] == "BOTH"]
        for _, row in both[both["paciente_norm"] != both["nome_beneficiario_norm"]].iterrows():
            logger.warning(
                f"divergencia de nome: {row['id_cobranca']} | "
                f"excel={row['paciente']} | csv={row['nome_beneficiario']}"
            )

    def _log_ans_divergences(self) -> None:
        both = self.df[self.df["fonte"] == "BOTH"]
        for _, row in both[both["registro_ans"] != both["ans"]].iterrows():
            logger.warning(
